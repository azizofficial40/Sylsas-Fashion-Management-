...
const SalesAnalytics: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    const calculateTotal = (filtered: Order[]) => filtered.reduce((acc, o) => acc + (o.totalAmount || 0), 0);

    return {
      today: calculateTotal(orders.filter(o => new Date(o.date) >= todayStart)),
      yesterday: calculateTotal(orders.filter(o => {
        const d = new Date(o.date);
        return d >= yesterdayStart && d < todayStart;
      })),
      week: calculateTotal(orders.filter(o => new Date(o.date) >= weekStart)),
      month: calculateTotal(orders.filter(o => new Date(o.date) >= monthStart)),
  ...
  const stats = useMemo(() => {
    ...
    const productsSold = filteredOrders.reduce((acc, o) => {
      return acc + o.items.reduce((iAcc, item) => iAcc + (item.quantity || 0), 0);
    }, 0);
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-none">{t.title}</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 px-2">
        {[
          { label: t.summary.today, value: summary.today, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30' },
          { label: t.summary.yesterday, value: summary.yesterday, icon: Calendar, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-950/30' },
          { label: t.summary.week, value: summary.week, icon: TrendingUp, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-950/30' },
          { label: t.summary.month, value: summary.month, icon: ShoppingBag, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-950/30' },
          { label: t.summary.lifetime, value: summary.lifetime, icon: DollarSign, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30' },
        ].map((card, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-3">
            <div className={`w-10 h-10 ${card.bg} ${card.color} rounded-xl flex items-center justify-center`}>
              <card.icon size={20} />
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{card.label}</p>
              <p className={`text-xl font-black ${card.color}`}>৳{card.value.toLocaleString()}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">{t.chart}</h3>
        ...
        <div className="space-y-4">
          {[
            { label: t.stats.totalOrders, value: stats.totalOrders, icon: ShoppingBag, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-950/30' },
            { label: t.stats.revenue, value: `৳${stats.revenue.toLocaleString()}`, icon: DollarSign, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30' },
            { label: t.stats.productsSold, value: stats.productsSold, icon: Archive, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30' },
          ].map((stat, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50">
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.table.id}</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.table.customer}</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.table.product}</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t.table.qty}</th>
              ...
              {filteredOrders.length > 0 ? filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-5">
                    <span className="text-xs font-black text-slate-900 dark:text-white">#{order.id.slice(-6).toUpperCase()}</span>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-1">
                      {order.items.map((item, idx) => (
                        <span key={idx} className="text-[10px] font-bold text-slate-600 dark:text-slate-400 line-clamp-1">
                  ...
                  <td className="px-6 py-5">
                    <span className="text-sm font-black text-indigo-600 dark:text-indigo-400">৳{order.totalAmount.toLocaleString()}</span>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                      order.status === 'Delivered' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30' :
                      order.status === 'Cancelled' ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/30' :
                      order.status === 'Pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/30' :
